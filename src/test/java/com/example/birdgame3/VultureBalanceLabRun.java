...
/**
 * Manual focused balance audit for Vulture tuning changes.
 * Deliberately excluded from the default Surefire naming patterns.
 *
 * Run with:
 *   .\mvnw.cmd test -Dtest=VultureBalanceLabRun
 */
class VultureBalanceLabRun {

    @Test
    void runFocusedVultureBalanceLab() {
        BirdStats.reloadFromDisk();
        BirdGame3 game = new BirdGame3(Preferences.userRoot().node(
                "/birdfight3-tests/vulture-balance-lab/" + UUID.randomUUID()));
        BirdGame3.BirdType focus = BirdGame3.BirdType.VULTURE;
        long seed = 20260808L;
        int wins = 0;
        int decided = 0;
        int matches = 0;
        ...
        System.out.printf(Locale.ROOT,
                "[vulture-balance] %s: %d/%d wins (%.1f%%), %d matches%n",
