...
/**
 * Manual focused balance audit for recovery-sensitive Eagle and Raven changes.
 * Deliberately excluded from the default Surefire naming patterns.
 *
 * Run with:
 *   .\mvnw.cmd test -Dtest=RecoveryBalanceLabRun
 */
class RecoveryBalanceLabRun {

    @Test
    void runFocusedRecoveryBalanceLab() {
        BirdStats.reloadFromDisk();
        BirdGame3 game = new BirdGame3(Preferences.userRoot().node(
                "/birdfight3-tests/recovery-balance-lab/" + UUID.randomUUID()));
        BirdGame3.BirdType[] focusBirds = {
                BirdGame3.BirdType.EAGLE,
                BirdGame3.BirdType.RAVEN
        };
        long seed = 20260808L;

        for (BirdGame3.BirdType focus : focusBirds) {
            int wins = 0;
            int decided = 0;
            int matches = 0;
            ...
            System.out.printf(Locale.ROOT,
                    "[recovery-balance] %s: %d/%d wins (%.1f%%), %d matches%n",
                    focus.name, wins, decided, decided == 0 ? 0.0 : wins * 100.0 / decided, matches);
        }
    }
}
