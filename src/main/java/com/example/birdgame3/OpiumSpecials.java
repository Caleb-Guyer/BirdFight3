...
final class OpiumSpecials {
    ...
    static void emitBurst(Bird bird, double originX, double originY, int count, Color color) {
        for (int i = 0; i < bird.scaledParticleCount(count); i++) {
            ...
            double speed = 1.2 + bird.game.nextParticleRandom() * 5.8;
            bird.game.particles.add(new Particle(
                    originX + Math.cos(angle) * (8.0 + bird.game.nextParticleRandom() * 18.0),
                    originY + Math.sin(angle) * (8.0 + bird.game.nextParticleRandom() * 18.0),
    ...
    static void handleStatusEffects(Bird bird) {
        if (bird.opiumDrowsyTimer > 0) {
            ...
            if ((bird.opiumDrowsyTimer & 7) == 0) {
                bird.game.particles.add(new Particle(
                        bird.bodyCenterX() + (bird.game.nextParticleRandom() - 0.5) * bird.bodyWidth() * 0.86,
                        bird.bodyCenterY() - 18.0 * bird.sizeMultiplier
                                + (bird.game.nextParticleRandom() - 0.5) * bird.bodyHeight() * 0.32,
                        (bird.game.nextParticleRandom() - 0.5) * 1.2,
        ...
        if (bird.heisenBrittleTimer > 0 && (bird.heisenBrittleTimer & 5) == 0) {
            bird.game.particles.add(new Particle(
                    bird.bodyCenterX() + (bird.game.nextParticleRandom() - 0.5) * bird.bodyWidth() * 0.78,
                    bird.bodyCenterY() + (bird.game.nextParticleRandom() - 0.5) * bird.bodyHeight() * 0.58,
                    (bird.game.nextParticleRandom() - 0.5) * 1.8,
                    -0.9 - bird.game.nextParticleRandom() * 1.5,
            ...
            for (int i = 0; i < Bird.HEISEN_ULTIMATE_SHARD_COUNT; i++) {
                ...
                if ((bird.heisenUltimateTimer & 1) == 0) {
                    bird.game.particles.add(new Particle(
                            ...
                            -0.6 - bird.game.nextParticleRandom() * 1.2,
    ...
    static void handleState(Bird bird) {
        ...
        if (bird.opiumSideTimer > 0) {
            ...
            if (bird.opiumSideFueled && (bird.opiumSideTimer & 1) == 0) {
                bird.game.particles.add(new Particle(
                        ...
                        bird.bodyCenterY() + (bird.game.nextParticleRandom() - 0.5) * 34.0 * bird.sizeMultiplier,
                        -bird.opiumSideDirection * (1.0 + bird.game.nextParticleRandom() * 2.4),
                        (bird.game.nextParticleRandom() - 0.5) * 1.8,
        ...
        if (bird.opiumUpTimer > 0) {
            ...
            if (bird.opiumUpFueled && (bird.opiumUpTimer & 1) == 0) {
                bird.game.particles.add(new Particle(
                        bird.bodyCenterX() + (bird.game.nextParticleRandom() - 0.5) * 32.0 * bird.sizeMultiplier,
                        bird.bodyBottomY() - 4.0 * bird.sizeMultiplier,
                        (bird.game.nextParticleRandom() - 0.5) * 1.8,
                        2.0 + bird.game.nextParticleRandom() * 3.0,
    ...
    private static void handleHeisenTrap(Bird bird, Bird.OpiumTrap trap, Iterator<Bird.OpiumTrap> it) {
        if (birdStandingInTrap(bird, bird, trap, 74.0, 42.0)) {
            ...
            if ((trap.ageFrames & 5) == 0) {
                bird.game.particles.add(new Particle(
                        bird.bodyCenterX() + (bird.game.nextParticleRandom() - 0.5) * 28.0 * bird.sizeMultiplier,
        ...
        if ((trap.ageFrames & 3) == 0) {
            bird.game.particles.add(new Particle(
                    trap.x + (bird.game.nextParticleRandom() - 0.5) * 42.0,
                    trap.y - 20.0 - bird.game.nextParticleRandom() * 34.0,
                    (bird.game.nextParticleRandom() - 0.5) * 0.8,
    ...
    private static void handleOpiumTrap(Bird bird, Bird.OpiumTrap trap, Iterator<Bird.OpiumTrap> it) {
        ...
        if (birdStandingInTrap(bird, bird, trap, 108.0, 42.0)) {
            ...
            if ((trap.ageFrames & 7) == 0) {
                bird.game.particles.add(new Particle(
                        ...
                        (bird.game.nextParticleRandom() - 0.5) * 0.7,
                        -0.5 - bird.game.nextParticleRandom(),
        ...
        if ((trap.ageFrames & 7) == 0) {
            bird.game.particles.add(new Particle(
                    trap.x + (bird.game.nextParticleRandom() - 0.5) * 88.0,
                    trap.y - 8.0 - bird.game.nextParticleRandom() * 18.0,
                    (bird.game.nextParticleRandom() - 0.5) * 0.9,
                    -0.5 - bird.game.nextParticleRandom() * 1.2,
