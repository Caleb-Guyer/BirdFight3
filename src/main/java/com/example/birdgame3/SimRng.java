 ...
 *
 * <p>All gameplay randomness (knockback variance, particle spawn angles, AI jitter,
 * hazard timing) must come from this single stream so that a match can be reproduced
 * from its seed plus the frame-by-frame inputs. The stream is reseeded at the start
 * of every match via {@link #reseed(long)}; consumption outside a match (menus,
 * victory screens) is therefore harmless.
 *
